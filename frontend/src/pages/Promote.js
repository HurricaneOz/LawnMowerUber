...
export default function Promote() {
  const [formData, setFormData] = useState({ service_id: "", rate: "" });
  ...
  const handleEventClick = (clickInfo) => {
    if (window.confirm(`Remove this slot from ${clickInfo.event.start.toLocaleString()}?`)) {
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("providers", JSON.stringify([...existing, payload]));

    alert("Provider info saved!");
